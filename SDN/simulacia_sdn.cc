...
int main (int argc, char *argv[])
{
    ...
    //// SDN config
    ////////////////////////////////////////////////////////////////////////////////////
    // SDN - vytvorenie switchu pre kazdy node 
    NodeContainer switches;
    switches.Create (nnodes);

    //SDN - Use the CsmaHelper to connect hosts and switches
    ...
    switchPorts [2] = NetDeviceContainer ();
    switchPorts [3] = NetDeviceContainer ();

      // Connect host 0 to first switch
    pair = NodeContainer (nodes.Get (0), switches.Get (0));
    ...
    pair = NodeContainer (nodes.Get (1), switches.Get (1));
    pairDevs = csmaHelper.Install (pair);
    hostDevices.Add (pairDevs.Get (0));
    switchPorts [1].Add (pairDevs.Get (1));


    pair = NodeContainer (nodes.Get (2), switches.Get (2));
    pairDevs = csmaHelper.Install (pair);
    hostDevices.Add (pairDevs.Get (0));
    switchPorts [2].Add (pairDevs.Get (1));


    pair = NodeContainer (nodes.Get (3), switches.Get (3));
    pairDevs = csmaHelper.Install (pair);
    hostDevices.Add (pairDevs.Get (0));
    ...
    switchPorts [1].Add (pairDevs.Get (1));
      ...
      // Connect the switches
    pair = NodeContainer (switches.Get (0), switches.Get (1), switches.Get (2), switches.Get (3));
    pairDevs = csmaHelper.Install (pair);
    switchPorts [0].Add (pairDevs.Get (0));
    switchPorts [1].Add (pairDevs.Get (1));
    switchPorts [2].Add (pairDevs.Get (2));
    switchPorts [3].Add (pairDevs.Get (3));
...
/*
    ...
    */
    ...
    csmaHelper.EnablePcap ("switch", switchPorts [3], true);
    csmaHelper.EnablePcap ("host", hostDevices);

    AnimationInterface anim ("simulaciasSDN.xml");
